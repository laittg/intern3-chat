...
export const chatPOST = httpAction(async (ctx, req) => {
  ...
  // Prepare mutation args with conditional edit parameters
  ...
  // Only add edit parameters if they exist in the request
  if (body.editMode === true) {
    mutationArgs.editMode = true;
  }
  if (body.editFromMessageId && typeof body.editFromMessageId === "string") {
    mutationArgs.editFromMessageId = body.editFromMessageId;
  }
